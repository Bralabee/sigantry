...
| Exit `2` on every run -- "workspace not found" | The workspace GUID changed (e.g. workspace recreated) or the runner identity lacks workspace-level access. | Validate via `sigantry workspace list --workspace-id <id>` from the runner; grant `Member` or `Contributor` to the runner identity. |
