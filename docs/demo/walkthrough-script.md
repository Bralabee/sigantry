# Sigantry walkthrough -- script (mkdocs-served mirror)

> Phase 15 / DEMO-03. This file mirrors `scripts/remotion/script.md`
> so outside readers can read the walkthrough's narrative without
> rendering the mp4. The Remotion build (Plan 15-02) reads the
> canonical source at `scripts/remotion/script.md`; edits should land
> there and propagate via `mkdocs build` (or be byte-copied to this
> file per release per the `## Source` footer).
...
Edit `scripts/remotion/script.md` (the canonical source) -- re-run
`scripts/build-walkthrough.sh` (Plan 15-02) to regenerate the mp4.
Sync this mkdocs mirror per release with:

```bash
cp scripts/remotion/script.md docs/demo/walkthrough-script.md
# (then re-prepend the mkdocs header above the canonical content)
```
